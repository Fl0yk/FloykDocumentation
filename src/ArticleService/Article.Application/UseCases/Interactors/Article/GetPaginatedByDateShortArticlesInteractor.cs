    ...
    : IRequestHandler<GetPaginatedByDateShortArticlesRequest, PaginatedResult<ShortArticleDTO>>
{
    ...
    {
        var items = await _unitOfWork.ArticleRepository.GetPaginatedByDateWithoutBlocksArticles(request.PageNo, request.PageSize, cancellationToken);
        ...
        return new()
        {
            Items = _mapper.Map<IEnumerable<ShortArticleDTO>>(items),
